...
func main() {
    ...
    if err := InitMasterDB(); err != nil { log.Fatalf("Erro init master DB: %v", err) }
    if err := RunMasterMigrations(); err != nil { log.Fatalf("Erro migrações master: %v", err) }
    if err := SeedSuperAdmin(); err != nil { log.Fatalf("Erro seed superadmin: %v", err) }
