...
function toChart(series) {
  ...
  return series.slice().sort((a, b) => a.ts - b.ts).map((p) => ({ ts: p.ts, rxBps: p.rxBps ?? 0, txBps: p.txBps ?? 0 }));
}
function tickTime(ts) {
  const d = new Date(ts);
  return d.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" });
}
export function NetworkUsageCard({ current, series }) {
        ...
        {data.length === 0 ? (
          ...
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={data} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
              ...
              <Tooltip contentStyle={{ background: "rgba(2,6,23,0.95)", border: "1px solid rgba(255,255,255,0.1)" }} labelFormatter={(v) => tickTime(Number(v))} formatter={(v, name) => [formatBps(Number(v)), name === "rxBps" ? "RX" : "TX"]} />
              <Legend formatter={(v) => (v === "rxBps" ? "RX (Entrada)" : "TX (Saída)")} wrapperStyle={{ color: "rgba(226,232,240,0.75)", fontSize: 12 }} />
